package com.main_components 
{
	...
	import com.components.PseudoMultiSheetMusicDisplay;
	...
	public class PitchTracker extends Sprite 
	{
		...
		private var sheetDisplay:PseudoMultiSheetMusicDisplay;
		...
		private var _multipitchVect:Vector.<Vector.<Note>>;
		...
		private function init(e:Event = null):void 
		{
			...
			_multipitchVect = new Vector.<Vector.<Note>>;
			...
			sheetDisplay = new PseudoMultiSheetMusicDisplay(_w - 50, _h * 0.75, _maxErrorCents);
		...
		private function vectorUpdateDisplays(perc:Number):void
		{
			...
			var multipitchSamplesToShow:Vector.<Vector.<Note>> =  new Vector.<Vector.<Note>>(_currentSamplesShown);
			...
			for ( var i:uint = 0; i < _currentSamplesShown; i++)
			{
				try //DONE : take in account if sharp or flat to change the history according to that
				{
					...
					multipitchSamplesToShow[i] = _multipitchVect[i + _currentInitSample];
					...
					//TODO change name for multipitch samples
				...
				{//out of bounds; complete with null
					...
					multipitchSamplesToShow[i] = new Vector.<Note>();
				}
			...
			sheetDisplay.showVector(multipitchSamplesToShow, marksToShow);
		...
		{	
			...
			//if ( spect.freq > MIN_FREQ )//&& spect.freq < MAX_FREQ)
			{
				...
				var multipitch:Vector.<Note> = new Vector.<Note>;
				for each(var mn:Number in e.multipitch)
				{
					noteMapper.updateFreq(mn);
					multipitch.push(noteMapper.getNote());
				}
				//
				if (_showSharpNotes)
				{
				...
				if (_notesVect.length > _maxSamplesHistory)
				{
					...
					_notesVect.shift();
					_multipitchVect.shift();
					_samplesCount--;
				}
				_ticksBooleanVect.push(tick);
				_notesVect.push(n);
				
				_multipitchVect.push(multipitch);
				...
				//sheetDisplay.update(n, tick);
				sheetDisplay.update(multipitch, tick);
