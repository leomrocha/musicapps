package com.components 
{
	 ...
	 * @author Leo Panigo
	 */
	public class PseudoMultiSheetMusicDisplay extends Sprite 
	{
		...
		private var _currentNotesVect:Vector.<Vector.<Note>>; //current notes being displayed
		...
		private var _displayNotesVect:Vector.<Vector.<PseudoMusicSheetNote>>;
		
		private var _yValsVect:Vector.<Vector.<Number>>; //current values to show, normalized to 1
		private var _coloursVect:Vector.<Vector.<uint>>; //colours to show, for each element in the previousvector: MUST have the same amount of elements
		
		private var _numberNotes:uint = 3;
		 ...
		 * @param	
		 */
		public function PseudoMultiSheetMusicDisplay(w:uint = 640 , h:uint = 480,
													...
													showFreq:Boolean = true, showNote:Boolean =true)
		{
			...
			//calculate current 
			_currentSamplesShown = 125;// _w - 30; //WARNING, TODO: this is fixed like this only for the moment, then should be calculated dinamically!!!
			_currentNotesVect = new Vector.<Vector.<Note>>(_numberNotes);
			for (i = 0; i < _numberNotes; i++)
			{
				_currentNotesVect[i] = new Vector.<Note>(_currentSamplesShown); 
			}
			
			_displayNotesVect = new Vector.<Vector.<PseudoMusicSheetNote>>(_numberNotes);
			for (i = 0; i < _numberNotes; i++)
			{
				_displayNotesVect[i] = new Vector.<PseudoMusicSheetNote>(_currentSamplesShown);
			}
			
			_yValsVect = new Vector.<Vector.<Number>>(_numberNotes);
			for (i = 0; i < _numberNotes; i++)
			{
				_yValsVect[i] = new Vector.<Number>(_currentSamplesShown); 
			}
			
			_coloursVect = new Vector.<Vector.<uint>>(_numberNotes);
			for (i = 0; i < _numberNotes; i++)
			{
				_coloursVect[i] = new Vector.<uint>(_currentSamplesShown); 
			}
		...
		private function sheetLinesMove8(dir:int = -1):void
		{
			...
			//clefG.x = 10;
			clefG.y += dir*(8 * _ystep);
			//addChild(clefG);
			
			//clefF.x = 30;
			clefF.y += dir*(8 * _ystep);
			//addChild(clefF);
		...
		public function clear():void
		{
			...
			for (i = 0; i < _numberNotes; i++)
			{
				for each( var mn:PseudoMusicSheetNote in _displayNotesVect[i])
				{
					try{
					if (mn.parent) removeChild(mn);
					}catch(e:Error){}
				}
			}
			_displayNotesVect = new Vector.<Vector.<PseudoMusicSheetNote>>(_numberNotes);
			...
			//_linesVector = new Vector.<Shape>;
			//_namesVector = new Vector.<TextField>;
			update(new Vector.<Note>,false);
		}
		
		//TODO correct for multipitch
		public function showVector( notes:Vector.<Vector.<Note>>, marks:Vector.<Boolean>=null):void
		{
			...
			{
				for each(var vn:Vector.<Note> in notes)
				{
					update(vn,false);//TODO also is missing the metronome ticks vector!!
				}
			}
			
		}
		
		public function update(notes:Vector.<Note>, tick:Boolean ):void
		{
			trace("update multi sheet: ", notes.length);
			for (var k:uint = 0; k < notes.length; k++)
			{
				
				var note:Note = notes[k];
				trace("drawing note at update: ", k, note);
				...
				_currentNotesVect[k].shift();
				_currentNotesVect[k].push(note);
				...
				var dn:PseudoMusicSheetNote = new PseudoMusicSheetNote(note, _xstep*(2 -0.3*k), c, drawLine);
				dn.x = xpos;
				dn.y = ypos;
				dn.alpha = 1 - (k * 0.2 );
				
				var prevN:PseudoMusicSheetNote = _displayNotesVect[k].shift();
				try {
						removeChild(prevN);
				}catch ( e:Error)
				{
				...
				if (k == 0 && note.freq >= MIN_FREQ && note.freq <= MAX_FREQ)
				{
				...
				//notes
				for each(var np:PseudoMusicSheetNote in  _displayNotesVect[k])//move notes
				{
					if (np)
					{
						np.x -= _xstep;
			...
			if (tick)
			{
				var mMark:SimpleLine = new SimpleLine(_h - 100);
				mMark.x = xpos;
				mMark.y = 50;
