package com.leopanigo.components.displays.bmp_displays 
{
	 ...
	 * @author Leo Panigo
	 */
	public class BMPAmplitudGraph extends Bitmap
	{ // from http://wonderfl.net/c/heMs
        private var _w:uint,//width  with a rate of 11 this should be 512
					...
					_pointWidth:Number,
					_maxVal:Number = 1.0;
        public function BMPAmplitudGraph(width:uint = 512, height:uint = 211, maxVal:Number=1.0, pointWidth:Number = 3, waveColour:uint = 0xAAffAA, backgroundColour:uint = 0x000000):void{
        //public function BMPAmplitudGraph(width:uint = 512, height:uint = 211, pointWidth:Number = 2, waveColour:uint = 0xAAffAA, backgroundColour:uint = 0x000000):void{
            //_maxVal = maxVal;
			_w = width;
        ...
        public function drawGraph(valsVect:Vector.<Number>, colourVect:Vector.<uint>):void {
			...
			for (var i:uint = 0 ; i < valsVect.length; i++ )
			{
                ...
                _sampleRect.y = _offset - (valsVect[i] * _a);//_offset - _a * _sn / _maxVal;
