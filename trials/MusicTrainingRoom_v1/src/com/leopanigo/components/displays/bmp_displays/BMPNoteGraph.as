package com.leopanigo.components.displays.bmp_displays 
{
	 ...
	 * @author Leo Panigo
	 */
	public class BMPNoteGraph extends Bitmap
	{ // from http://wonderfl.net/c/heMs
        private var _w:uint,//width  with a rate of 11 this should be 512
                    ...
                    _n:int,
					_maxVal:Number = 1.0;
        public function BMPNoteGraph(width:uint = 512, height:uint = 211, maxVal:Number=1, pointWidth:Number = 1, waveColour:uint = 0xAAffAA, backgroundColour:uint = 0x000000):void{
            _maxVal = maxVal;
			_w = width;
		...
		//input should be from interval [0,2] all other values are not accepted and will give some weird behaviour
		//
		public function drawGraph(valsVect:Vector.<Number>):void {
			...
			for each (var v:Number in valsVect)
			{
				...
				yamp = Math.log(1+v);// results between 
                _sampleRect.y = _offset -  _a * yamp;//_offset - _a * _sn / _maxVal;
				_sampleRect.height =  _a * yamp * 2;
				_wc = ((0xff * yamp) << 16);// + 0x0000cc * Math.log(v); //assuming 1 is the highest value and 12 the lowest
