package com.leopanigo.audio_processing
{
     ...
     */
        public class SpectrumAnalyzer extends EventDispatcher
        {
            ...
            //private var SAMPLE_RATE:Number = 5512;   // Actual microphone sample rate (Hz)
            //private var SAMPLE_RATE:Number = 8000;   // Actual microphone sample rate (Hz)
            private var SAMPLE_RATE:Number = 11025;   // Actual microphone sample rate (Hz)
            //private var SAMPLE_RATE:Number = 22050;   // Actual microphone sample rate (Hz)
            //private var SAMPLE_RATE:Number = 44100;   // Actual microphone sample rate (Hz)
            private var LOGN:uint = 16;              // Log2 FFT length
            private var N:uint = 1 << LOGN;           // FFT Length
            //private var NS:uint = 1024;               //number of samples for each 
            private var BUF_LEN:uint = N //NS;             // Length of buffer for mic audio
            private var WIN_LEN:uint = 2048;
            private var UPDATE_PERIOD:int = 30//130;       // Period of spectrum updates (ms)

            private var BIN_RESOLUTION:Number = SAMPLE_RATE/N; // resolution of each bin in frequency;
            //private var NEW_BIN_RESOLUTION:Number = (SAMPLE_RATE/5)/N; // resolution of each bin in frequency;
			...
			private var DETECTION_THRESHOLD:Number = 30; //Detection threshold .. yet to tune it
			...
			//private var multiPitchLevel:uint;
			...
			public function reconfigure(params:SpectAnalyzerParametrization):void
			{
				//stop everything
				stop();
				//clear old things
				try {
					...
					m_mic.removeEventListener( SampleDataEvent.SAMPLE_DATA, onMicSampleData );
					
				}catch (e:Error) {
					//no event exists. no need to do anything
				}
				...
				HPS_LEVEL = params.HPS_LEVEL;
				DETECTION_THRESHOLD = params.DETECTION_THRESHOLD;
				//and now all the calculations about other things: booleans to activate and desactivate different processing treatements TODO
				
				//restart all the elements
				init();
            ...
            */
            ...
            private function updateSpectrum( event:Event ):void
            {
				...
				dispatchEvent(new SpectrumEvent(m_currVolume, f0));
            }
