package com.audio_processing
{
     ...
     * Calculates the FFT, |FFT|, HPS, approximation of the Constant Q Transform and the resulting chromagram
     *
     ...
     * IN THE SOFTWARE.
     */
        public class SpectrumAnalyzerNew extends EventDispatcher
        {
			...
			private var makeChroma:Boolean = true; //CURRENTLY in development, calculate the chromagram, for that needs the 
             ...
             * 
             */     
            public function SpectrumAnalyzerNew()
            {
                var i:uint;
            ...
            */
			...
			private function updateXqt():void
			{
				...
				for each(var xqt:XqtElement in m_xqt)
				{
					...
					{
						//trace("suming", qsum, m_hps[i], m_maxnorm);
						//qsum = qsum + (m_hps[i] / m_maxnorm); //only normalize the values for the sum this avoids number overflow AND keeps divisions to a minimum
						qsum = qsum + (m_hps[i]); //only normalize the values for the sum this avoids number overflow AND keeps divisions to a minimum
					}
					//qsum = qsum + (m_hps[xqt.pos] / m_maxnorm); //WARNING, this does not take in account misstunings and inharmonicity factor, check previous for cycle for that, this is only a test
					
					trace("qsum = ", qsum);
					xqt.value = qsum;
					//update chromagram now
					
					var notepos:uint = noteNameChromaPosMap[xqt.noteName];//TODO BUG here, always gives 0
            ...
            private function updateSpectrum( event:Event ):void
            {
				...
				//if (makeHPS){ //always true for the moment
					...
					for (i = 0; i< lenMag/HPS_LEVEL; i++)
					{
						...
						{
							//if(m_hps[i] > 0.0 && m_hps[i*j] > 0.0) //is already non negative
							//{
								m_hps[i] = m_hps[i]* m_hps[i*j];
							//}
						}
						if (m_hps[i] > m_maxnorm) //for normalization later
						{
				...
				dispatchEvent(new SpectrumEvent(m_currVolume, f0, m_chromagram));
            }
